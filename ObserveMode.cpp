...
static Load< MeshBuffer > meshes(LoadTagDefault, []() -> MeshBuffer const * {
	MeshBuffer *ret = new MeshBuffer(data_path("cubes.pnct"));
...
static Load< Scene > scene(LoadTagLate, []() -> Scene const * {
	Scene *ret = new Scene();
	ret->load(data_path("cubes.scene"), [](Scene &scene, Scene::Transform *transform, std::string const &mesh_name){
...
void ObserveMode::draw(glm::uvec2 const &drawable_size) {
	...
	{ //help text overlay:
		...
		std::string help_text = "--SWITCH CAMERAS WITH LEFT/RIGHT, PRESS ENTER FOR ANSWER--";
		if (see_answer) {
			help_text = "THERE SHOULD BE 69 CUBES YOU CAN SEE.";
		}
		glm::vec2 min, max;
		draw.get_text_extents(help_text, glm::vec2(0.0f, 0.0f), 0.5f, &min, &max);
		float x = std::round(160.0f - (0.5f * (max.x + min.x)));
		draw.draw_text(help_text, glm::vec2(x, 1.0f), 0.5f, glm::u8vec4(0x00,0x00,0x00,0xff));
		draw.draw_text(help_text, glm::vec2(x, 2.0f), 0.5f, glm::u8vec4(0xff,0xff,0xff,0xff));
	}
}
